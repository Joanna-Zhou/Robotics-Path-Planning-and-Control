...
class FeatureProcessor:
    ...
    def get_image(self, id):
        ...
        img = cv2.imread(os.path.join(self.data_folder,
                                      'camera_image_{}.jpeg'.format(id)))
    ...
    def append_matches(self, matches, new_kp):
        ...
        if len(self.feature_match_locs) == 0:
            ...
            self.feature_match_locs = np.append(
                self.feature_match_locs, np.array([cur_feature_match_locs]), axis=0)
            print("self.feature_match_locs", self.feature_match_locs.shape)
...
def triangulate(feature_data, tf, inv_K):
    ...
    for i in range(len(feature_data)):
        # print(feature_data[i], np.array([-1,-1]))
        if np.all(feature_data[i] != np.array([-1, -1])):
            ...
            feature_aug = np.ones(3)
            feature_aug[:2] = feature_data[i]
            inv_R = np.linalg.inv(tf[i])[:3, :]
            v = inv_R.T @ inv_K @ feature_aug
            vvT = v @ v.T
            factor1 += np.identity(3) - vvT
            factor2 += (np.identity(3) - vvT) @ camera_center
        else:
            print("feature_data[i]", feature_data[i])

    p = np.linalg.inv(factor1) @ factor2

    return p


def main():
    # minimum number of images a feature must be seen in to be considered useful
    min_feature_views = 20
    K = np.array([[530.4669406576809, 0.0, 320.5],  # K from sim
                  [0.0, 530.4669406576809, 240.5],
                  [0.0, 0.0, 1.0]])
    # will be useful for triangulating feature locations
    ...
    # output shape should be (num_images, num_features, 2)
    feature_locations = f_processor.get_matches()
    ...
    ax.scatter(pc[:, 0], pc[:, 1], pc[:, 2], marker='^',
               color=colors, label='features')

    ax.scatter(traj_pc[:, 0], traj_pc[:, 1], traj_pc[:, 2],
               marker='o', label='robot poses')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    ...
    ax.scatter(good_feature_locations[0, :, 0],
               good_feature_locations[0, :, 1], marker='^', color=colors)
